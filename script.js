(() => {
    ...
    const finalScreenHeader = finalScreen.getElementsByClassName('js-final-screen__header')[0];
    const finalScreenButton = finalScreen.getElementsByClassName('js-final-screen__button')[0];
    ...
    const renderView = () => {
        const renderAlphabet = () => {
            alphabetElement.innerHTML = allLetters.map((letter) => (
                `<button class="js-alphabet-letter alphabet__letter"${state.used.includes(letter) ? ' disabled' : ''}>
                    ${letter}
                </button>`
            )).join('');
        };
        ...
        const renderWord = () => {
            ...
            wordElement.innerHTML = state.word.split("").map((letter, index, allLetters) => {
        ...
        const renderFinalScreen = () => {
            ...
            if (wordWithoutFirstAndLastLetter.split('').every((letter) => state.used.includes(letter))) {
                finalScreen.classList.remove(hiddenClassName)
    ...
    const fetchNewWord = async () => {
        try {
            const response = await fetch("http://hangman.career.wecode.stage.wecode.agency/api/v1/word/random");
